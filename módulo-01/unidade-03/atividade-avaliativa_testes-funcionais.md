...
| - Integração com serviço de autenticação | Consulta de produtos disponíveis | 
| - Integração com serviço de pedidos | Adição e remoção de itens no carrinho |
|   | Cálculo automático do valor total do pedido | Usuários não autenticados não podem finalizar pedidos |
|   | Finalização do pedido com confirmação | Após a confirmação, o pedido não pode ser alterado |
